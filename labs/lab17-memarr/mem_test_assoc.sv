...
// SYSTEMVERILOG: void function
    ...
    begin
        ...
        a_arr[memrnd.addr] = memrnd.data;
    end
    $display("Will Check: %0d", a_arr.num());
    if(assocarr.first(idx))
      do begin
         mbus.read_mem(idx, rdata, 1);
         error_status = checkit(idx, rdata, a_arr[idx]);
         coverrand.sample();
         a_arr.delete(idx);
      end
      while (a_arr.next(idx));
    $display("memory locations left in array:%0d", a_arr.num());
    printstatus(error_status);
