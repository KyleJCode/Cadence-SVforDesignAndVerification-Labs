...
// SYSTEMVERILOG: void function
    ...
    memrnd.cntrl = uclc;
    $display("Array Size = %0d", d_arr.size());
    for (int i = 0; i< 32; i++)
    begin
       ok = memrnd.randomize();
       mbus.write_mem(memrnd.addr, memrnd.data, 1);
       d_arr[memrnd.addr] = memrnd.data;
    end

    for (int i = 0; i< 32; i++)
    begin
       if (d_arr[i] === 8'hxx)
          $display("No Data at %0d", i);
       else begin
         mbus.read_mem  (i, rdata, 1);
         error_status = checkit (i, rdata, d_arr[i]);
         coverrand.sample();
       end
    end
    printstatus(error_status);
    d_arr.delete();
    $display("Dynamic array size:%d", d_arr.size());
    $finish;
  end
